...
def locationsJSON():
    ...
    return jsonify(Restaurants=[location.serialize for location in
                   locations])


@app.route('/locations/byname/JSON')
@app.route('/locations/byname/asc/JSON')
def restaurantsByNameAscJSON():
...
def restaurantMenuJSON(location_id):
...
def deleteLocation(location_id):
    ...
    if deletedLocation.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to delete this location. Please create your own restaurant in order to delete.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def newMenuItem(location_id):
    ...
    if login_session['user_id'] != location.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add menu items to this location. Please create your own restaurant in order to add items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def editMenuItem(location_id, menu_id):
    ...
    location = session.query(Location).filter_by(id=location_id).one()
    if login_session['user_id'] != location.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit menu items to this location. Please create your own restaurant in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(location_id, menu_id):
    ...
    if login_session['user_id'] != location.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete menu items to this location. Please create your own restaurant in order to delete items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
